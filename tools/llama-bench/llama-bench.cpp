...
static cmd_params parse_cmd_params(int argc, char ** argv) {
    ...
    params.bit_attn             = cmd_params_defaults.bit_attn;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        try {
            if (arg == "--bit-attn" || arg == "--no-bit-attn") {
                params.bit_attn = arg == "--bit-attn";
                continue;
            }
            if (arg == "-h" || arg == "--help") {
...
struct test {
    ...
    static const std::vector<std::string> & get_fields() {
        static const std::vector<std::string> fields = {
            ...
            "main_gpu",       "no_kv_offload",  "flash_attn",    "bit_attn",        "devices",        "tensor_split",
    ...
    static field_type get_field_type(const std::string & field) {
        ...
        if (field == "f16_kv" || field == "no_kv_offload" || field == "cpu_strict" ||
            field == "embeddings" || field == "no_host" || field == "bit_attn") {
            return BOOL;
        }
